...
const GET_USERS_BY_RESTAURANT = gql`
  query GetUsersByRestaurant($restaurantId: String!) {
    usersByRestaurant(restaurantId: $restaurantId) {
      id
      name
      email
    }
  ...
  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 hover:shadow-md transition-all">
    <div className="flex items-start justify-between mb-3">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <div className="flex-shrink-0 h-12 w-12 bg-orange-100 dark:bg-orange-900 rounded-full flex items-center justify-center">
...
const UsersByRestaurantPage = () => {
  ...
  // Query para obtener usuarios por restaurante
  const {
    data: usersData,
    loading: usersLoading,
    error: usersError,
  } = useQuery(GET_USERS_BY_RESTAURANT, {
    variables: { restaurantId: userData?.restaurantId || '' },
    skip: !userData?.restaurantId || !isClient,
  });

  // Obtener usuarios
  const users: User[] = usersData?.usersByRestaurant || [];
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 sm:p-6 flex items-center justify-center">
        <div className="text-center">
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6">
            <UtensilsCrossed className="mx-auto h-12 w-12 text-yellow-600 dark:text-yellow-400 mb-4" />
            <h3 className="text-lg font-semibold text-yellow-800 dark:text-yellow-400 mb-2">
              No hay restaurante asignado
            </h3>
            <p className="text-yellow-600 dark:text-yellow-400">
              El usuario no tiene un restaurante asignado en el sistema.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                {userData?.restaurantName || `Restaurant ID: ${userData.restaurantId}`}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <KPICard
            icon={Building2}
            title="Restaurant ID"
            value={userData.restaurantId?.slice(-4) || '----'}
          />
        </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              {searchTerm
                ...
                : 'Este restaurante no tiene usuarios asignados.'}
            </p>
          </div>
        ) : (
          <>
...
export default UsersByRestaurantPage;
