...
const GET_TOURISM_COMPANY = gql`
  query GetTourismCompany($id: ID!) {
    tourismCompany(id: $id) {
      id
      name
      description
      city
...
export default function TourismDetailPage() {
  ...
  const { data, loading, error } = useQuery(GET_TOURISM_COMPANY, {
    variables: { id: companyId },
    skip: !companyId,
  });

  const [updateCompany] = useMutation(UPDATE_TOURISM_COMPANY);
  const [formData, setFormData] = useState<any>({});
  const [activeTab, setActiveTab] = useState('details');
  const [saveStatus, setSaveStatus] = useState<'idle' | 'saving' | 'success' | 'error'>('idle');

  useEffect(() => {
    if (data?.tourismCompany) {
      setFormData(data.tourismCompany);
    }
  }, [data]);
  ...
  const handleSave = async () => {
    ...
    try {
      const { id, plans, createdAt, updatedAt, __typename, ...inputData } = formData;
  ...
  const tabs = [
    ...
    { id: 'plans', label: 'Planes', icon: List },
  ];
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">No se encontró el ID de la empresa</p>
        </div>
      </div>
    );
  }
  ...
  const inputClassName =
    'w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent';
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                  Configuración de Empresa Turística
                </h1>
                        ...
                        <div className="space-y-6">
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              rows={4}
                              className={inputClassName}
                              placeholder="Describe tu empresa turística..."
                            />
                          </div>
                      ...
                      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                        {formData.plans && formData.plans.length > 0 ? (
                          ...
                          <div className="text-center text-gray-500 dark:text-gray-400 py-8">
                            No hay planes turísticos disponibles
                          </div>
                        )}
                      </div>
                    </div>
