...
const GET_USERS_BY_SERVICE_PROVIDER = gql`
  query GetUsersByServiceProvider($serviceProviderId: String!) {
    usersByServiceProvider(serviceProviderId: $serviceProviderId) {
      id
      name
      email
    }
  ...
  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 hover:shadow-md transition-all">
    <div className="flex items-start justify-between mb-3">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <div className="flex-shrink-0 h-12 w-12 bg-purple-100 dark:bg-purple-900 rounded-full flex items-center justify-center">
...
const UsersByServicePage = () => {
  ...
  // Query para obtener usuarios por proveedor de servicios
  const {
    data: usersData,
    loading: usersLoading,
    error: usersError,
  } = useQuery(GET_USERS_BY_SERVICE_PROVIDER, {
    variables: { serviceProviderId: userData?.serviceProviderId || '' },
    skip: !userData?.serviceProviderId || !isClient,
  });

  // Obtener usuarios
  const users: User[] = usersData?.usersByServiceProvider || [];
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 sm:p-6 flex items-center justify-center">
        <div className="text-center">
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6">
            <Briefcase className="mx-auto h-12 w-12 text-yellow-600 dark:text-yellow-400 mb-4" />
            <h3 className="text-lg font-semibold text-yellow-800 dark:text-yellow-400 mb-2">
              No hay servicio asignado
            </h3>
            <p className="text-yellow-600 dark:text-yellow-400">
              El usuario no tiene un servicio asignado en el sistema.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                {userData?.serviceName || `Service ID: ${userData.serviceProviderId}`}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <KPICard
            icon={Building2}
            title="Service ID"
            value={userData.serviceProviderId?.slice(-4) || '----'}
          />
        </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              {searchTerm
                ...
                : 'Este servicio no tiene usuarios asignados.'}
            </p>
          </div>
        ) : (
          <>
...
export default UsersByServicePage;
